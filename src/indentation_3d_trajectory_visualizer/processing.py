...
def detect_load_candidate_regions(df: pd.DataFrame, config: VisualizationConfig) -> list[TrialRegion]:
    ...
    regions: list[TrialRegion] = []
    for start, end in merged:
        if end - start + 1 < min_samples:
            continue
        contact_start = find_contact_start_index(df, start, config)
        cut_start = max(0, contact_start - pre_samples)
        cut_end = min(len(df) - 1, end + post_samples)
        trial_id = len(regions) + 1
        regions.append(
            TrialRegion(
                trial_id=trial_id,
                ...
                cut_start_index=cut_start,
                cut_end_index=cut_end,
...
def find_contact_start_index(df: pd.DataFrame, threshold_start_index: int, config: VisualizationConfig) -> int:
    ...
    load = df["load_smooth_detection_N"].to_numpy()
    contact_start = threshold_start_index
    for idx in range(threshold_start_index, -1, -1):
        if load[idx] <= contact_threshold:
            contact_start = idx
            break
...
def zero_adjust_trial(
    ...
    if baseline_part.empty:
        baseline_part = trial.iloc[: max(1, contact_start_local)]
...
def make_trial_dataframe(
    ...
    *,
    ...
    trial["relative_time_s"] = trial["global_time_s"] - df.loc[region.contact_start_index, "global_time_s"]
    trial["active_flag"] = (
    ...
    max_load = float(trial.loc[trial["active_flag"], "load_zeroed_N"].max())
    displacement_excursion = float(
        trial.loc[trial["active_flag"], "indentation_mm"].max()
        - trial.loc[trial["active_flag"], "indentation_mm"].min()
    )
    active_duration = float(
        df.loc[region.active_end_index, "global_time_s"] - df.loc[region.active_start_index, "global_time_s"]
    )
    quality = label_event_quality(max_load, displacement_excursion, active_duration, config)
...
def label_event_quality(
    max_load: float,
    displacement_excursion_mm: float,
    active_duration_sec: float,
    config: VisualizationConfig,
) -> str:
    if max_load < config.load_threshold_n:
        return "low_load"
    if active_duration_sec < config.min_active_duration_sec:
        return "short_event"
    if displacement_excursion_mm < 0.05:
...
def process_dataframe(
    ...
    *,
    ...
    regions = detect_load_candidate_regions(preprocessed, config)
